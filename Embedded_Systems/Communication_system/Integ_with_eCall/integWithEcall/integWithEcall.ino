...
const char* dmss = "https://178.32.101.106:3000/api/dms";         // For dms flag
...
void sendFlagDmsToBackend(byte status) {
  if (WiFi.status() == WL_CONNECTED) {
    WiFiClientSecure client;
    client.setInsecure(); // Accept all SSL certs

    HTTPClient http;
    http.begin(client, dmss);
    http.addHeader("Content-Type", "application/json");
    ...
    if (httpResponseCode > 0) {
      String response = http.getString();
      Serial.print("Dms Flag Sent, Response: ");
      Serial.println(response);
    } else {
      Serial.print("Dms Flag Send Failed, Error code: ");
      Serial.println(httpResponseCode);
    }

    http.end();
  } else {
    Serial.println("WiFi not connected. Can't send dms flag.");
  }
}
...
sendFlagDmsToBackend(0); //dms flag
...
*/                      
  ...
  if (Serial1.available()) {
    ...
    switch (i2c_flag) {
      case 7: //Dms
      Serial1.print('4'); 
      sendFlagDmsToBackend(1); //send flag to backend 
      while(1);
      break;
      case 8: //red
      Serial1.print('4'); 

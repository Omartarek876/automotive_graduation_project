...
# Load models
DMS_model1 = YOLO("/home/mahmoud/integration_V2/Mask_ncnn_model")  
DMS_model2 = YOLO("/home/mahmoud/integration_V2/best_ncnn_model") 
nvs_model1 = YOLO("/home/mahmoud/integration_V2/yolov8n_ncnn_model")
nvs_model2 = YOLO("/home/mahmoud/integration_V2/traffic_ncnn_model")
...
mixer.init()
voice_left = mixer.Sound('/home/mahmoud/integration_V2/left.wav')
voice_right = mixer.Sound('/home/mahmoud/integration_V2/Right.wav')
voice_down = mixer.Sound('/home/mahmoud/integration_V2/down.wav')
eyes_blink= mixer.Sound('/home/mahmoud/integration_V2/eyes_blink.wav')
...
# Function to send flag via SPI
def send_spi_flag(flag):
    flag_dict = {
        "LEFT_ALERT": 0xA1,
        "RIGHT_ALERT": 0xA2,
        "DOWN_ALERT": 0xA3,
        "UP_ALERT": 0xA4,
        "EYES_CLOSED_ALERT": 0xA5
    }
    flag_value = flag_dict.get(flag, 0x00)  # Default to 0x00 if flag not found
    spi.xfer([flag_value])  # Send flag over SPI
    print(f"SPI Flag Sent: {flag}")

"""
def send_alert(flag):
        send_uart_flag(flag)  # Send flag via UART
        send_spi_flag(flag)   # Send flag via SPI
"""
...
def check_traffic_signal(label):
    if label in ["red", "green", "stop"]:
        print(f"Detected traffic signal: {label}")
        try:
            ser.write(label.encode() + b"\n")
            print(f"Sent flag: {label}")
        except Exception as e:
...
def main():
    ...
    while True:
        ...
        ret, nvs_frame = cap.read()
        if not ret:
            print("Error: Couldn't read frame from USB camera.")
            break
        nvs_frame = cv2.resize(nvs_frame, (640, 640))  # Resize nvs_frame to 640x640
        ...
        try:
            results1 = DMS_model1(frame, imgsz=480)
            results2 = DMS_model2(frame, imgsz=480)
            ...
            cv2.putText(frame, case, (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            
            # Night Vision Processing
            nvs_results1 = nvs_model1(nvs_frame, imgsz=480)
            nvs_results2 = nvs_model2(nvs_frame, imgsz=480)
            ...
            for r1 in nvs_results1:
                for box in r1.boxes:
                    ...
                    cv2.rectangle(nvs_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    cv2.putText(nvs_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            ...
            for r2 in nvs_results2:
                for box in r2.boxes:
                    ...
                    cv2.rectangle(nvs_frame, (x1, y1), (x2, y2), color, 2)
                    cv2.putText(nvs_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                    check_traffic_signal(label)

            cv2.imshow("Driver Monitoring System", frame)
            cv2.imshow("Night Vision System", nvs_frame)

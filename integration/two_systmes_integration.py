...
# --------------------------
# Main Loop: Integration of DMS and NVS 
# --------------------------

def main():
    # Initialize the PiCamera for DMS
    picam2 = Picamera2()
    camera_config = picam2.create_preview_configuration(main={"size": (720, 720), "format": "RGB888"})
    picam2.configure(camera_config)
    picam2.start()
    
    # Initialize the USB camera for NVS
    cap = cv2.VideoCapture(2)
    
    # Load YOLO models
    model_mask = YOLO("/home/mahmoud/integration/Mask_ncnn_model")
    model_face = YOLO("/home/mahmoud/integration/best_ncnn_model")
    model_night = YOLO("/home/mahmoud/integration/night_ncnn_model")
    
    while True:
        # ----- Driver Monitoring System (PiCamera) -----
        frame_dms = picam2.capture_array()
        if frame_dms is None:
            print("DMS frame not captured")
            continue
        
        results_mask = model_mask(frame_dms,imgsz=480)
        results_face = model_face(frame_dms,imgsz=480)
        case = check_conditions(results_mask, results_face)
        
        # Draw YOLO detections manually to avoid errors
        for result in [results_mask, results_face]:
            for box in result[0].boxes:
                ...
                cv2.rectangle(frame_dms, (x1, y1), (x2, y2), (0,0,255), 2)
        
        if case == "Face detected":
            frame_dms = Head_Eye_detect(frame_dms)
        elif case == "Face with glasses detected":
            frame_dms = detect_head_pose(frame_dms)
        
        cv2.putText(frame_dms, case, (10, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, GREEN, 2)
        cv2.imshow("Face Detection", frame_dms)
        
        # ----- Night Vision System (USB Camera) -----
        ret, frame_nvs = cap.read()
        if ret:
            results_nvs = model_night(frame_nvs,imgsz=480)
            annotated_frame = results_nvs[0].plot()
            cv2.imshow("NightVision System", annotated_frame)
        else:
            print("NVS frame not captured")
        
        # Break loop if 'q' is pressed (applies to both windows)
